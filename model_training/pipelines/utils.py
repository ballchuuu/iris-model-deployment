...
def train_test_split_by_species(
    ...
    """
    Function performs train test split by species to ensure that each species
    has some rows in train set and test set
...
def push_metric_to_db(
    db_name: str, model_name: str, train_acc: float, test_acc: float
) -> bool:
    """
...
def train_model(
    ...
    model_name: str,
    db_name: str = "../../data/database.sqlite"
) -> sklearn.base.BaseEstimator:

    """
    Function is model agnostic and takes in any sklearn model for training.
    After training the weights are saved in the form of a pickle file with the
    metrics saved to the SQLite database
    ...
    pushed_bool, model_ver = push_metric_to_db(
        db_name=db_name, model_name=model_name, train_acc=train_acc, test_acc=test_acc
    )
    if not pushed_bool:
    ...
    # save model to pickle file under assets
    # filename is: model_name + model_id to uniquely differentiate the weights
